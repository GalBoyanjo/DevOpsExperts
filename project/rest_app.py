...
def user(user_id):
    if request.method == 'POST':
        if get_user(USERNAME, PASSWORD, HOST, user_id) == None:
            # getting the json data payload from request
            request_data = request.json
            # treating request_data as a dictionary to get a specific value from key
            user_name = request_data.get('user_name')
            add_user(USERNAME, PASSWORD, HOST, user_id, user_name)
        ...
        if get_user(USERNAME, PASSWORD, HOST, user_id) != None:
            ...
            # treating request_data as a dictionary to get a specific value from key
            user_name = request_data.get('user_name')
            update_user(USERNAME, PASSWORD, HOST, user_id, user_name)
        ...
        user_name = get_user(USERNAME, PASSWORD, HOST, user_id)
        if get_user(USERNAME, PASSWORD, HOST, user_id) != None:
            delete_user(USERNAME, PASSWORD, HOST, user_id)
